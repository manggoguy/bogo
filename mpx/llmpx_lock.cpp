...
private:
    ...
    std::map<std::pair<Value *, Instruction *>,
             std::list<Value *> *>
        gv_bound_checklist_cache;
    std::map<std::pair<Value *, Instruction *>,
             std::list<Value *> *>
        gv_key_checklist_cache;
...
Value *llmpx::get_infinite_bound(Function *f)
{
    ...
    Instruction *insertpoint = dyn_cast<Instruction>(f->getEntryBlock().getFirstInsertionPt());
...
void llmpx::create_llmpx_symbols(Module &module)
{
    ...
    //func void test()
    {
        FunctionType *_llmpx_test_fntype = FunctionType::get(VoidTy, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "test",
            _llmpx_test_fntype);
        _llmpx_test = module.getFunction(LLMPX_SYMBOL_PREFIX "test");
        assert(_llmpx_test && "wtf, _llmpx_test is null, this is impossible!");
    }
    ...
    //func void temporal_chk(VoidPtrTy, Int64Ty)
    {
        ...
        FunctionType *_llmpx_temporal_chk_fntype = FunctionType::get(VoidTy, plist, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "temporal_chk",
            _llmpx_temporal_chk_fntype);
        _llmpx_temporal_chk = module.getFunction(LLMPX_SYMBOL_PREFIX "temporal_chk");
        assert(_llmpx_temporal_chk && "wtf, _llmpx_temporal_chk is null, this is impossible!");
    }
    //func Int64Ty temporal_lock_alloca(VoidPtrTy, Int64Ty)
    {
        ...
        FunctionType *_llmpx_temporal_lock_alloca_fntype = FunctionType::get(Int64Ty, plist, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "temporal_lock_alloca",
            _llmpx_temporal_lock_alloca_fntype);
        _llmpx_temporal_lock_alloca = module.getFunction(LLMPX_SYMBOL_PREFIX "temporal_lock_alloca");
        assert(_llmpx_temporal_lock_alloca && "wtf, _llmpx_temporal_lock_alloca is null, this is impossible!");
    }
    //func void temporal_key_store(VoidPtrTy, Int64Ty)
    {
        ...
        plist.push_back(VoidPtrTy); //ptr
        plist.push_back(Int64Ty);   //key
        FunctionType *_llmpx_temporal_key_store_fntype = FunctionType::get(VoidTy, plist, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "temporal_key_store",
            _llmpx_temporal_key_store_fntype);
        _llmpx_temporal_key_store = module.getFunction(LLMPX_SYMBOL_PREFIX "temporal_key_store");
        assert(_llmpx_temporal_key_store && "wtf, _llmpx_temporal_key_store is null, this is impossible!");
    }
    //func Int64Ty temporal_key_load(VoidPtrTy)
    {
        std::vector<Type *> plist;
        plist.push_back(VoidPtrTy); //ptr
        FunctionType *_llmpx_temporal_key_load_fntype = FunctionType::get(Int64Ty, plist, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "temporal_key_load",
            _llmpx_temporal_key_load_fntype);
        _llmpx_temporal_key_load = module.getFunction(LLMPX_SYMBOL_PREFIX "temporal_key_load");
        assert(_llmpx_temporal_key_load && "wtf, _llmpx_temporal_key_load is null, this is impossible!");
    }
    //func void _llmpx_dbg_dump_bndldstx(VoidPtrTy, Int1Ty)
    {
        ...
        FunctionType *_llmpx_dbg_dump_bndldstx_fntype = FunctionType::get(VoidTy, plist, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "dbg_dump_bndldstx",
            _llmpx_dbg_dump_bndldstx_fntype);
        _llmpx_dbg_dump_bndldstx = module.getFunction(LLMPX_SYMBOL_PREFIX "dbg_dump_bndldstx");
        assert(_llmpx_dbg_dump_bndldstx && "wtf, _llmpx_dbg_dump_bndldstx is null, this is impossible!");
    }
}
...
/*
*   create TxHook intrinsic symbols
*/
void llmpx::create_TxHook_function(Module &module)
{
    ...
    wrapper_mutex_lock = Function::Create(mutex_lock_type,
                                    GlobalValue::ExternalLinkage,
                                    "wrapper_mutex_lock",
                                    &module);
    FunctionType *mutex_unlock_type = FunctionType::get(VoidType, /* No ArgTypes,*/ false);
    //TxHookTxBegin = cast<Function > (module.getOrInsertFunction("TxHookTxBegin", TxHookTxBeginType));
...
void llmpx::create_llmpx_bnd_cache_symbols(Module &module)
{
     ...
     */
    {
        ...
        FunctionType *_llmpx_bnd_cache_demand_fntype = FunctionType::get(VoidPtrTy, plist, false);
        module.getOrInsertFunction(
            LLMPX_SYMBOL_PREFIX "bnd_cache_demand",
            _llmpx_bnd_cache_demand_fntype);
        _llmpx_bnd_cache_demand = module.getFunction(LLMPX_SYMBOL_PREFIX "bnd_cache_demand");
        assert(_llmpx_bnd_cache_demand && "wtf, _llmpx_bnd_cache_demand is null, this is impossible!");
    }
}
...
void llmpx::create_llmpx_wrapper_symbols(Module &module)
{
         ...
         it != end; ++it)
    {
         ...
         */
        Function *wrapper_func = Function::Create(func_type,
                                                  func_ptr->getLinkage(),
         ...
         */
        FunctionType *new_func_type = FunctionType::get(ret_type,
                                                        params,
                                                        func_type->isVarArg());
        //Create cast from wrapper_func to function of new_func_type
        Value *new_func = ConstantExpr::getPointerCast(wrapper_func, new_func_type->getPointerTo());
#if DEBUG_WRAPPER_PROCESSOR
...
void llmpx::create_global_constants(Module &module)
{
    ...
    bnd_invalid = new GlobalVariable(module,
                                     ArrayTy,
                                     false,
                                     GlobalValue::LinkOnceAnyLinkage,
                                     0, "llmpx_bnd_invalid");
    ...
    key_anyvalid = new GlobalVariable(module, Int64Ty,
    ...
    key_anyinvalid = new GlobalVariable(module, Int64Ty,
...
bool llmpx::runOnModule(Module &module)
{
     ...
     */
    create_TxHook_function(module);
    /*
    * create TxHook function
    */
...
llmpx::associate_meta(Instruction *I, Value *V)
{
    ...
    if (isa<GlobalVariable>(V))
    {
        ...
        {
            if (dyn_cast<GlobalVariable>(V)->getLinkage() != GlobalValue::ExternalLinkage)
            {
                V->dump();
...
llmpx::insert_bound_load(Instruction *I, Value *ptr, Value *ptrval)
{
    ...
    if (llmpx_bound_cache)
    {
        ...
        Instruction *bnd_cache_addr = CallInst::Create(_llmpx_bnd_cache_demand, bnd_cache_args, "", I);
...
llmpx::insert_key_load(Instruction *I, Value *ptr)
{
    ...
    std::vector<Value *> args;
    args.push_back(addr);
    Instruction *keyldx = CallInst::Create(_llmpx_temporal_key_load, args,
                                           "keyldx", I);
...
llmpx::insert_bound_store(Instruction *I, Value *ptr, Value *ptrval, Value *bnd)
{
    ...
    if (llmpx_bound_cache)
    {
        ...
        Instruction *bnd_cache_addr = CallInst::Create(_llmpx_bnd_cache_demand,
...
llmpx::insert_key_store(Instruction *I, Value *ptr, Value *key)
{
    ...
    Instruction *keystx = CallInst::Create(_llmpx_temporal_key_store, args,
                                           "", insertPoint);
...
llmpx::insert_dbg_dump_bndldstx(Instruction *I, Value *ptr, bool is_load)
{
    ...
    std::vector<Value *> args;
    args.push_back(ptr);
    args.push_back(is_load ? ConstantInt::getTrue(*ctx)
                           : ConstantInt::getFalse(*ctx));

    Instruction *dbg_dump_bndldstx_call = CallInst::Create(_llmpx_dbg_dump_bndldstx, args, "", I);
...
llmpx::insert_bound_check(Instruction *I, Value *ptr, bool is_load)
{
    ...
    {
        ConstantExpr *constant = dyn_cast<ConstantExpr>(ptr);
        Instruction *inst = constant->getAsInstruction();
    ...
    args5.push_back(bnd);
    uint64_t rwsize = module->getDataLayout()
                          .getTypeStoreSizeInBits(ptr->getType()->getContainedType(0)) /
                      8;
...
llmpx::insert_check(Instruction *I, Value *ptr, bool is_load)
{
    ...
    if (llmpx_enable_temporal_safety)
    {
        std::list<Value *> ilist2 = insert_keylock_check(I, ptr, is_load);
...
Value *llmpx::get_bound(Value *v, Instruction *I)
{
    ...
    if (isa<ConstantExpr>(v))
    {
        ...
        if (gv_bound_checklist_cache.find(std::make_pair(v, I)) == gv_bound_checklist_cache.end())
        {
        ...
        {
            cblist = gv_bound_checklist_cache[std::make_pair(v, I)];
            return cblist->back();
        }

        ConstantExpr *constant = dyn_cast<ConstantExpr>(v);
        Instruction *inst = constant->getAsInstruction();
        Value *addr;
        if (isa<GetElementPtrInst>(inst))
        {
        ...
        {
            if (dyn_cast<GlobalVariable>(addr)->getLinkage() != GlobalValue::ExternalLinkage)
            {
                addr->print(errs());
                llvm_unreachable("non-external global not found"
                                 " in gv_bound_checklist?");
...
Value *llmpx::get_key(Value *v, Instruction *I)
{
    ...
    if (isa<ConstantExpr>(v))
    {
        ...
        if (gv_key_checklist_cache.find(std::make_pair(v, I)) == gv_key_checklist_cache.end())
        {
        ...
        IRBuilder<> builder(I);
        Value *gvkey_ptr = builder.CreateBitCast(raw_gvkey, Type::getInt64PtrTy(*ctx));
        Value *key = builder.CreateLoad(Type::getInt64Ty(*ctx), gvkey_ptr, "");
...
void llmpx::transform_functions(Module &module)
{
         ...
         it != orig_flist.end(); ++it)
    {
...
#endif
            AttributeSet ret_attr = AttributeSet::get(func_ptr->getContext(),
             ...
             i != e; ++i, ++arg_index)
        {
            ...
            {
                AttributeSet attrs = orig_attr.getParamAttributes(arg_index);
                AttrBuilder B(orig_attr, arg_index);
                AttributeSet one_arg_attr = AttributeSet::get(func_ptr->getContext(),
                                                              params.size(), B);
                if (arg_is_pointer)
                {
                    ...
                    {
                        one_arg_attr = one_arg_attr
                                           .removeAttribute(*ctx,
                                                            arg_index,
                                                            Attribute::Returned);
                    }
                    if (one_arg_attr.getAttribute(arg_index,
                                                  Attribute::ByVal) != Attribute())
                    {
             ...
             arg_i != arg_e; ++arg_i, ++attr_index)
        {
            ...
            if (isa<PointerType>(arg_orig->getType()))
            {
                ...
                {
                    AttributeSet attrs = newfunc_attr.getParamAttributes(attr_index);
...
/*
    ...
    for (GlobalVariable &gvi : module.globals())
    {
        ...
        GlobalVariable *gvbnd = new GlobalVariable(module,
                                                   ArrayTy,
                                                   false,
                                                   gi->getLinkage(),
                                                   0, "llmpx_bnd_" + gvname);
        ...
        indic_lb.push_back(ConstantInt::get(int64ty, 0));
        Constant *bnd_lb = ConstantExpr::getGetElementPtr(NULL, gvbnd, indic_lb);

        Constant *ub = ConstantExpr::getNeg(
            ConstantExpr::getAdd(
        ...
        Constant *bnd_ub = ConstantExpr::getGetElementPtr(
            NULL,
            gvbnd,
            indic_ub);
        ...
        if (llmpx_enable_temporal_safety)
        {
            ...
            Value *key = builder.CreateCall(_llmpx_temporal_lock_alloca,
                                            args, "llmpx_key." + gvname);

            auto *Int64Ty = Type::getInt64Ty(*ctx);

            GlobalVariable *gvkey = new GlobalVariable(module, Int64Ty,
                                                       false, gi->getLinkage(),
                                                       0, "llmpx_key_" + gvname);
...
void llmpx::process_each_function(Module &module)
{
         ...
         f_begin != f_end; ++f_begin)
    {
        ...
        bool found = (std::find(std::begin(processed_flist),
                                std::end(processed_flist), func_ptr) != std::end(processed_flist));
        if (found)
        {
            continue;
        }
        ...
        if (new_func_ptr != func_ptr)
        {
                 ...
                 (i != e) && (idx < bnd_idx); ++i, ++idx)
            {
                if (isa<PointerType>(i->getType()))
                {
                    ...
                    auto ptrbyval_it = std::find(ptrbyval_list->begin(),
                                                 ptrbyval_list->end(),
                                                 idx);
                    if (ptrbyval_it != ptrbyval_list->end())
                    {
                        ...
                        Constant *dist_arg_for_bndmk = ConstantInt::get(Type::getInt64Ty(*ctx), (allocated_size - 1));
                        args_for_bndmk.push_back(dist_arg_for_bndmk);

                        Instruction *bndmkcall = init_builder.CreateCall(mpx_bndmk, args_for_bndmk,
                     ...
                     (i != e) && (idx < bnd_idx); ++i, ++idx)
                {
                    if (isa<PointerType>(i->getType()))
                    {
                        ...
                        if (ptrbyval_it != ptrbyval_list->end())
                        {
                            ...
                            Constant *dist_arg_for_kl_alloca = ConstantInt::get(Type::getInt64Ty(*ctx), (allocated_size - 1));
                            args_for_kl_alloca.push_back(dist_arg_for_kl_alloca);

                            Instruction *lock_alloca_call = init_builder.CreateCall(_llmpx_temporal_lock_alloca,
                                                                                    args_for_kl_alloca,
...
void llmpx::cleanup(Module &module)
{
    ...
    for (auto I : flist_orig)
    {
        ...
        std::string old_func_name = std::string(old_func_ptr->getName().data());
         ...
         mi != me; ++mi)
    {
             ...
             fi != fe; ++fi)
        {
                 ...
                 bi != be; ++bi)
            {
                ...
                if (std::find(std::begin(mpx_chk_intrinsics),
                              std::end(mpx_chk_intrinsics), fname) == std::end(mpx_chk_intrinsics))
                {
                    continue;
                }
        ...
        collect_all_def_use:
                 ...
                 bi != be; ++bi)
            {
                ...
                if (std::find(reorderlist.begin(), reorderlist.end(), cins) != reorderlist.end())
                {
...
Value *llmpx::handleAlloca(Value *ii)
{
    ...
    if (alloca_inst->hasNUsesOrMore(1))
    {
        ...
        std::vector<Value *> args;
        Value *ptr_arg_for_bndmk = builder.CreateBitCast(alloca_inst, Int8PtrTy,
        ...
        else
        {
            ...
            Value *allocated_size_0 = builder.CreateGEP(
...
//FIXME:should be allocated_size_0 - 1....
            ...
            Value *allocated_size_for_bndmk = builder.CreatePtrToInt(allocated_size_0,
                                                                     Type::getInt64Ty(*ctx));
        ...
        if (llmpx_enable_temporal_safety)
        {
            Instruction *lock = CallInst::Create(_llmpx_temporal_lock_alloca,
...
//need to propogate bound information from operand
    ...
    if ((operand->getType()->isPointerTy() && (!operand->getType()->getContainedType(0)->isFunctionTy())))
    {
        std::list<Value *> ilist = associate_meta(bitcast_inst, operand);
...
/*
    ...
    Value *new_bitcast_inst = builder.CreateBitCast(new_func, bitcast_inst->getType(), "");
...
Value *llmpx::handleCall(Value *ii)
{
    ...
    if (may_be_function_pointer && called_func && replace_func && (!has_wrapper))
    {
         ...
         */
        Type *good_return_type = the_new_func->getFunctionType()->getReturnType();
         ...
         */
        Type *nbtype = FunctionType::get(
                           good_return_type,
                           nb_rettype, the_new_func->isVarArg())
                           ->getPointerTo();
...
#endif
        IRBuilder<> builder(callinst);
        Value *new_called_value = builder.CreateBitCast(called_value, called_value_type, "");
    ...
    else
    {
        ...
        ret_val = ExtractValueInst::Create(nc, 0, "", GetNextInstruction(nc));
        blist->push_back(nc);
        if (llmpx_enable_temporal_safety)
        {
            klist->push_back(nc);
        ...
        {
            Value *ret_val_bnd = ExtractValueInst::Create(nc, 1, "",
            ...
            if (llmpx_enable_temporal_safety)
            {
                ...
                {
                    ret_val_key = ExtractValueInst::Create(nc, 2, "",
                                                           GetNextInstruction(dyn_cast<Instruction>(ret_val)));
                }
...
Value *llmpx::handleInvoke(Value *ii)
{
    ...
    else
    {
         ...
         * inst to the new BB
         */
        BasicBlock *newbb = BasicBlock::Create(*ctx, "msdist",
        ...
        //errs()<<"   return with bound, extract orig value into ret_val";
        ret_val = builder0.CreateExtractValue(nc, 0, basename + "_oret");
        blist->push_back(nc);
        if (llmpx_enable_temporal_safety)
        {
            klist->push_back(nc);
        ...
        if (isa<PointerType>(ret_val->getType()))
        {
            Value *ret_val_bnd = builder0.CreateExtractValue(nc, 1, basename + "_bnd");
            nblist->push_back(ret_val_bnd);
            if (llmpx_enable_temporal_safety)
            {
                Value *ret_val_key;
                ...
                {
                    ret_val_key = builder0.CreateExtractValue(nc, 2, basename + "_key");
                }
                nklist->push_back(ret_val_key);
            }
        }
...
//need to bitcast...
        ...
        ////////////////////////////////////////////////////
        BranchInst *brinst = builder0.CreateBr(normal_dest);
...
#else
    blist->push_back(NULL);
    std::list<Value *> ilist = associate_meta(gep, gep->getPointerOperand());
    blist->push_back(ilist.front());
...
Value *llmpx::handleLoad(Value *ii)
{
    ...
    if (GlobalVariable *po = dyn_cast<GlobalVariable>(ptr_operand))
    {
        if (po->getLinkage() == GlobalValue::ExternalLinkage)
        {
            ...
            if (isa<PointerType>(load_inst->getType()) && load_inst->hasNUsesOrMore(1))
            {
                Type *X86_BNDTy = Type::getX86_BNDTy(*ctx);
                PointerType *X86_BNDPtrTy = Type::getX86_BNDPtrTy(*ctx);
    ...
    if (val_type->isFloatingPointTy() || val_type->isFPOrFPVectorTy() || val_type->isVectorTy())
    {
        return ii;
    }
...
Value *llmpx::handlePHINode(Value *ii)
{
    ...
    if (this_phi_node_need_transform(phi_node))
    {
        ...
        for (int j = 0; j < incoming_val_cnt; j++)
        {
            ...
            if ((!!incom_func) && is_function_orig(incom_func))
            {
                ...
                Value *nfptr = builder.CreateBitCast(nfunc, incom_func->getType(), "");
    ...
    PHINode *bnd_phi_node = PHINode::Create(bndty,
                                            incoming_val_cnt,
...
Value *llmpx::handleBinaryOperator(Value *ii)
{
    ...
    ii->dump();
#endif
    Instruction *inst0 = dyn_cast<Instruction>(process_each_instruction(operand0));
    Instruction *inst1 = dyn_cast<Instruction>(process_each_instruction(operand1));
    Value *ptr = inst;
    //FIXME!
...
Value *llmpx::handleSelect(Value *ii)
{
    ...
    if ((t_val != proc_t_val) || (f_val != proc_f_val))
    {
        ...
        Value *newbitcast = builder.CreateSelect(cond,
                                                 proc_t_val, proc_f_val,
                                                 sel_inst->getName() + "_swbnd");
        Value *newresult = builder.CreateBitCast(newbitcast, sel_inst->getType(),
...
Value *llmpx::handleRet(Value *ii)
{
     ...
     */
    if ((!ret_val) ||
        ((!isa<PointerType>(ret_val->getType())) && (!ret_val->getType()->isStructTy())))
    {
        return ii;
    }
    Function *new_func = ret_inst->getParent()->getParent();
    //FIXME!
    Function *old_func = dyn_cast<Function>(find_nontransformed_function(new_func));
    if ((tr_flist_ret.find(old_func) == tr_flist_ret.end()) ||
        ((tr_flist_ret.find(old_func) != tr_flist_ret.end()) && (tr_flist_ret[old_func] <= 0)))
    {
    ...
    Instruction *i0 = InsertValueInst::Create(agg0, ret_val, 0, "mrv", ret_inst);
    iS.push_back(i0);
    kS.push_back(i0);
    new_ret_val = i0;
    ...
    if (isa<PointerType>(ret_val->getType()))
    {
        ...
        Instruction *i1 = InsertValueInst::Create(new_ret_val, bound,
                                                  1, "mrv", ret_inst);
        iS.push_back(i1);
        new_ret_val = i1;
        ...
        if (llmpx_enable_temporal_safety)
        {
            ...
            Instruction *i2 = InsertValueInst::Create(new_ret_val, key,
                                                      2, "mrv", ret_inst);
            kS.push_back(i2);
            new_ret_val = i2;
        }
    ...
    else if (ret_val->getType()->isStructTy())
    {
        ...
        for (int j = 0; j < ret_val_type->getStructNumElements(); j++)
        {
            ...
            if (etype->isPointerTy())
            {
                ...
                again:
                    ...
                    Value *the_ptr = GetElementPtrInst::Create(etype,
                                                               ret_val,
                                                               Idx,
                                                               "", ret_inst);
                ...
                else
                {
                    ...
                    while (!resolved)
                    {
                        ...
                        Instruction *chain_inst = dyn_cast<Instruction>(ret_chain);
                        if (!isa<InsertValueInst>(chain_inst))
                        {
                            goto again;
                        }
                        InsertValueInst *chain_iv_inst = dyn_cast<InsertValueInst>(chain_inst);
                        if (chain_iv_inst->getInsertedValueOperandIndex() == j)
                        {
                            resolved = true;
                            Value *the_ptr = chain_iv_inst->getInsertedValueOperand();
                            ptrlist.push_back(the_ptr);
        ...
        {
            Value *the_bnd = *it;
            Instruction *i1 = InsertValueInst::Create(new_ret_val, the_bnd,
        ...
        if (llmpx_enable_temporal_safety)
        {
            ...
            {
                Value *the_key = *it;
                Instruction *i1 = InsertValueInst::Create(new_ret_val, the_key,
                                                          meta_idx, "mrv", ret_inst);
                kS.push_back(i1);
                new_ret_val = i1;
                meta_idx++;
    ...
    ReturnInst *new_ret_inst = ReturnInst::Create(*ctx, new_ret_val, ret_inst);
#if (DEBUG_HANDLE_RET)
...
Value *llmpx::handleStore(Value *ii)
{
    ...
    if ((!val_type->isPointerTy()) && (stptr_type->isPointerTy() || act_stptr_type->isPointerTy()))
    {
        store_non_ptr_as_ptr = true;
    }
    if ((val_type->isPointerTy() || act_val_type->isPointerTy()) && (!stptr_type->isPointerTy()))
    {
    ...
    if (((!val_type->isPointerTy()) && (!act_val_type->isPointerTy()) && (!stptr_type->isPointerTy()) && (!act_stptr_type->isPointerTy())) || val_type->isFloatingPointTy() || val_type->isFPOrFPVectorTy() || val_type->isVectorTy())
    {
    ...
    else
    {
        ...
        //store the bound
        std::list<Value *> ins_blist = insert_bound_store(store_inst,
...
Type *llmpx::find_actual_type(Value *val, bool begin_from_ptr, int depth)
{
    ...
    while (!resolved)
    {
         ...
         */
        type = val->getType();
        if (type->isFPOrFPVectorTy() || type->isX86_MMXTy() || type->isX86_BNDTy() || type->isFunctionTy() || type->isVectorTy())
        {
            resolved = true;
            break;
        }
        ...
        default:
        {
            ...
            llvm_unreachable(ANSI_COLOR_RED
                             "WTF, can not handle" ANSI_COLOR_RESET);
            break;
        }
        }
...
bool llmpx::mpxTester(Module &module)
{
         ...
         f_begin != f_end; ++f_begin)
    {
        ...
        {
            Instruction *I = dyn_cast<Instruction>(II);
            if (isa<AllocaInst>(I) && I->getType()->isPointerTy())
            {
        ...
        while (bb_work_list.size())
        {
                 ...
                 ii != ie; ++ii)
            {
                ...
                store_check:
                    ...
                    if (isa<Function>(bare_val) || isa<ConstantPointerNull>(bare_val) || (!(isa<GlobalVariable>(bare_val) || isa<Instruction>(bare_val))) //function argument
                        || (isa<CallInst>(bare_val)) || (isa<PHINode>(bare_val)) || (isa<SelectInst>(bare_val)) || (isa<InvokeInst>(bare_val)))
                    {
